...
public partial class MainWindow : Window
{
    ...
    void ApplyStatus(JsonElement data){lastStatus=data.Clone();diagnostics=JsonSerializer.Serialize(data,new JsonSerializerOptions{WriteIndented=true});Diagnostics.Text=diagnostics;var b=data.TryGetProperty("backend",out var backend)?backend:data;
        ...
        bool checkedHardware=b.TryGetProperty("benchmark",out var check)&&check.ValueKind==JsonValueKind.Object&&check.TryGetProperty("complete",out var complete)&&complete.GetBoolean();
        checkedHardware|=(b.TryGetProperty("adapter",out var detectedAdapter)&&detectedAdapter.ValueKind==JsonValueKind.Null)||(b.TryGetProperty("core",out var detectedCore)&&detectedCore.ValueKind==JsonValueKind.Null);
        SetupBanner.Visibility=(neuralReady||checkedHardware)&&mediaReady?Visibility.Collapsed:Visibility.Visible;
        if(!setupRunning)SetupStatus.Text=neuralReady&&mediaReady?"Ready. Neural components, video tools and hardware checks are complete.":checkedHardware&&mediaReady?"Non-neural mode ready. This GPU did not pass neural evaluation. Save diagnostics below; clarity and color controls remain available.":"Click Complete setup to verify components and check compatibility.";
        ...
        if(!neuralReady&&b.TryGetProperty("failure_summary",out var failures)&&failures.GetArrayLength()>0){var first=failures[0];HardwareSummary.Text+="\nSize "+first.GetProperty("size").GetInt32()+": "+first.GetProperty("error").GetString();}
    ...
    void OnEvent(JsonElement message){if(!message.TryGetProperty("type",out var t))return;string type=t.GetString()??"";var data=message.TryGetProperty("data",out var d)?d:message;
        if(type=="complete"&&data.TryGetProperty("kind",out var setupKind)&&setupKind.GetString()=="setup"){SetBusy(false);ApplyStatus(data);Status.Text=SetupStatus.Text=data.TryGetProperty("setup_message",out var setupMessage)?setupMessage.GetString():"Setup complete · effects are OFF";return;}
